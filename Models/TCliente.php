...
trait TCliente{
	...
	public function setContacto(string $nombre, int $telefono, string $email, string $mensaje, string $ip, string $dispositivo, string $useragent){
		$this->con = new Mysql();
		...
		$query_insert  = "INSERT INTO contacto(nombre,telefono,email,mensaje,ip,dispositivo,useragent) 
						  VALUES(?,?,?,?,?,?,?)";
		$arrData = array($nombre,$telefono,$email,$mensaje,$ip,$dispositivo,$useragent);
		$request_insert = $this->con->insert($query_insert,$arrData);
		return $request_insert;
	}
}
